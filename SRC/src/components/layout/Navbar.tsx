  ...
  return (
    <header className="sticky top-0 z-30 w-full bg-[#080c14]/95 backdrop-blur-md border-b border-[#171f2f] shadow-tactical-md">
      ...
      <div className="w-full px-4 sm:px-6 lg:px-8 py-3 flex items-center justify-between gap-6">
        ...
        <div className="flex items-center gap-3.5 shrink-0">
          {onBackToLanding && (
            <button
              ...
              className="px-3 py-1.5 text-xs font-semibold rounded-xl bg-[#0f1728] text-blue-300 border border-blue-500/30 hover:bg-blue-600/20 hover:border-blue-500/50 transition-all flex items-center gap-1.5 shadow-sm"
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="hidden sm:inline-block px-1.5 py-0.5 text-[9px] font-mono font-bold rounded bg-blue-950/80 text-blue-400 border border-blue-500/30">
                PLATFORM CONSOLE
              </span>
            </div>
            <p className="text-[10px] text-slate-400 hidden lg:block tracking-wide uppercase font-mono">
              Smart Logistics & Accessibility Intelligence
            </p>
          </div>
        </div>

        {/* Zone 2 (Center): Unified Tactical Search & State Scope */}
        <div className="flex-1 max-w-2xl hidden md:flex items-center gap-2 bg-[#0a0e17] border border-[#1b2538] rounded-xl px-2 py-1 shadow-inner">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder="Filter corridors, passes (e.g. NH-6, Sonapur, Sela Pass)..."
              className="w-full pl-9 pr-3 py-1 text-xs bg-transparent text-slate-200 placeholder-slate-500 font-mono focus:outline-none"
            />
          </div>

          <div className="h-5 w-px bg-[#1d273a]" />
          ...
          <div className="relative shrink-0">
            <select
              ...
              className="appearance-none bg-[#0e1422] hover:bg-[#131c30] pl-3 pr-8 py-1.5 text-xs text-slate-200 rounded-lg border border-[#202d44] focus:outline-none cursor-pointer font-mono transition-colors"
            >
              {states.map((st) => (
        ...
        {/* Zone 3 (Right): Grouped Actions & Session Tools */}
        <div className="flex items-center gap-3 shrink-0">
          
          {/* Action Group: Emergency & Reporting */}
          <div className="flex items-center gap-2">
            {onOpenSirenModal && (
              <button
                onClick={onOpenSirenModal}
                className="px-3.5 py-1.5 rounded-xl bg-gradient-to-r from-rose-600 to-red-600 hover:from-rose-500 hover:to-red-500 text-white text-xs font-mono font-extrabold flex items-center gap-1.5 shadow-[0_0_15px_rgba(225,29,72,0.4)] transition-all animate-pulse"
            ...
            <button
              ...
              className="neu-btn-primary px-3.5 py-1.5 rounded-xl text-xs font-semibold flex items-center gap-1.5 transition-all"
              title="Report Incident or Road Washout"
            >
          ...
          {/* Action Group: Monitoring, Language, Fullscreen & Alerts */}
          <div className="flex items-center gap-2">
            <button
            ...
            <button
              ...
              className="neu-btn p-2 rounded-xl text-slate-300 hover:text-cyan-400 hover:border-cyan-500/40 transition-colors"
              title={isFullscreen ? "Exit Fullscreen Mode (Esc)" : "Full Screen Operations Mode"}
            >
              {isFullscreen ? (
        ...
        <div className="h-6 w-px bg-slate-800 hidden sm:block" />
          ...
          {currentUser ? (
            <div className="flex items-center gap-2.5">
              ...
              <button
                onClick={onSignOut}
                className="neu-btn px-3 py-1.5 rounded-xl text-xs font-medium text-slate-300 hover:text-rose-400 hover:border-rose-500/30 transition-colors"
            ...
            onOpenSignIn && (
              <button
                ...
                className="neu-btn px-3.5 py-1.5 rounded-xl text-xs font-semibold text-cyan-300 hover:text-white border border-cyan-500/30 transition-colors"
              >
                Sign In
              </button>
            )
